...
function getPostsForPage(pageNo) {
  // Probably don't need to have as var but just for testing purposes currently
  var postsPerPage = 2
  var err;
  var posts=getPosts();
  if(posts.length+1<postsPerPage*pageNo) {
      err = new Error('Not Found');
      err.status = 404;
  }
  return  {"posts" : posts.slice((pageNo-1)*postsPerPage,postsPerPage*pageNo), "maxPage": Math.ceil(posts.length/postsPerPage), "err":err};
}
